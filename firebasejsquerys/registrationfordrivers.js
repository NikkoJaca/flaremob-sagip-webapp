...
function addtodrivers() {
    ...
    let lisenum = document.getElementById("licensenum").value;
    let email = document.getElementById("emailadd").value;
    let password = document.getElementById("password").value;
    var drivers = database.child("tblDrivers/");
    ...
    drivers.push({
        drvFname:firstname,
        drvLname:lastname,
        drvLicenseNum:lisenum,
        drvEmailAdd:email,
        drvPassword:password,
    }).then(function() {
...
var selectedFile = null;
var extFile = null;
function DriverOpenImage(event){
    selectedFile = event.target.files[0]; 
    var reader = new FileReader();
  
    var imgtag = document.getElementById("DriverImg");
    ...
    if (extFile=="jpg" || extFile=="jpeg" || extFile=="png"){
      if (selectedFile.size>2097152){
        ...
        if (imgtag.title!='no_image.png'){
          ...
          drivertxtCompareName.value=DateTimeNow(9);
        }else{
    ...
    function FileOpenReset(){
      document.getElementById('DriverImageOpen').value='';
      document.getElementById('DriverImg').src='no_image.png';
    }
  }
  ...
  function addtodrivers(){
    ...
    var metaData={contentType: 'image/'+extFile, name:drivertxtCompareName.value};
      ...
      if(confMsg){    
        firebase.auth().onAuthStateChanged(function(user) {
          ...
          if (user) {
              ...
              var refFBStore=FBStore.child('driver_pictures/'+drivertxtCompareName.value).put(selectedFile,metaData)
              .then(snapshot => {
